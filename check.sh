...
sudo mysql -e "CREATE USER 'ubuntu'@'ip-${IP_ADDRESS//./-}' IDENTIFIED BY '123';"
sudo mysql -e "GRANT ALL PRIVILEGES on sakila.* TO 'ubuntu'@'ip-${IP_ADDRESS//./-}';"
sysbench --db-driver=mysql --mysql-db=sakila --mysql-user=ubuntu --mysql_password=123 --table-size=50000 --tables=10 /usr/share/sysbench/oltp_read_write.lua prepare
sysbench --db-driver=mysql --mysql-db=sakila --mysql-user=ubuntu --mysql_password=123 --table-size=50000 --tables=10 --threads=8 --max-time=20 /usr/share/sysbench/oltp_read_write.lua run | sudo tee -a mysql-cluster-results